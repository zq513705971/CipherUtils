...
namespace CipherUtils
{
    public class RsaKeyInfo
    {
        ...
        {
#if DES
            this.PublicKey = DESCode.DESDecrypt(dr["publickey"].ToString(), UserInfo.Prefix);
            this.PrivateKey = DESCode.DESDecrypt(dr["privatekey"].ToString(), UserInfo.Prefix);
#else
            this.PublicKey = AESCode.AESDecrypt(dr["publickey"].ToString(), UserInfo.Prefix);
            this.PrivateKey = AESCode.AESDecrypt(dr["privatekey"].ToString(), UserInfo.Prefix);
#endif
...
#if DES
            string _public = DESCode.DESEncrypt(key.PublicKey, UserInfo.Prefix);
            string _private = DESCode.DESEncrypt(key.PrivateKey, UserInfo.Prefix);
#else
            string _public = AESCode.AESEncrypt(key.PublicKey, UserInfo.Prefix);
            string _private = AESCode.AESEncrypt(key.PrivateKey, UserInfo.Prefix);
#endif
